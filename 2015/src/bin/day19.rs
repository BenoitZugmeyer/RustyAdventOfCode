...
impl Mutator {
            ...
            for target in targets {
                match reversed_mutations.entry(target.clone()) {
                    ...
                    btree_map::Entry::Occupied(_) => {
                        panic!("Mutation target {:?} is not unique.", target)
                    }
                }
            }
        }

        Mutator { reversed_mutations }
    }
    ...
    fn _find_min_steps(
        ...
        memoizer: &mut BTreeMap<Vec<Molecule>, Option<u32>>,
    ) -> Option<u32> {
        if let Some(result) = memoizer.get(target_molecule) {
            return result.to_owned();
        }

        let result = if target_molecule.is_empty() || target_molecule.len() < source_molecule.len()
        {
            None
            ...
            // Look ma, no loop!
            self.reversed_mutations
                .iter()
                .flat_map(|(source, target)| {
                    SubSlicesIterator::new(target_molecule, source)
                        .map(move |index| (source, target, index))
                })
                .filter_map(|(source, target, index)| {
                    let mut mutated_molecule =
                        Vec::with_capacity(target_molecule.len() - source.len() + 1);
        ...
        while let Some((molecule, steps)) = stack.pop_front() {
            ...
            stack.extend(
                self.reversed_mutations
                    .iter()
                    .flat_map(|(source, target)| {
                        SubSlicesIterator::new(&molecule, source)
                            .map(move |index| (source, target, index))
                    })
                    .filter_map(|(source, target, index)| {
                        ...
                        let mut mutated_molecule =
                            Vec::with_capacity(molecule.len() - source.len() + 1);
...
impl Elements {
    ...
    fn parse(&mut self, s: &str) -> Vec<u8> {
        ...
        RE.captures_iter(s)
            .map(|cap| self.parse_one(cap.at(0).unwrap()))
            .collect()
    }
...
fn specific_resolution(s: &str) -> u32 {
    Regex::new(r"[A-Z][a-z]*")
        .unwrap()
        .captures_iter(s)
        .fold(-1i32, |total, element| match element.at(0).unwrap() {
            "Rn" | "Ar" => total,
            "Y" => total - 1,
            _ => total + 1,
        }) as u32
...
fn main() {
    ...
    while let Some(line) = lines.next() {
        ...
        if let Some(index) = line.find(" => ") {
            ...
            mutations
                .get_default_mut(elements.parse_one(from))
    ...
    let maybe_min_steps = try_specific_solution(&molecule_line).or_else(|| {
