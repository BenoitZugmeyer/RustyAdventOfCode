...
impl Action {
    ...
    fn apply_binary(&self, lights: &mut [bool]) {
        self.apply_on_grid(lights, |light| match self.todo {
...
fn parse(s: &str) -> Option<Action> {
    ...
    RE.captures(s).map(|ref m| Action {
...
fn main() {
    ...
    println!(
        "Binary lights lit: {}",
        binary_lights.iter().filter(|&&on| on).count()
    );
    println!(
        "Multi level lights brightness: {}",
        multilevel_lights
            .iter()
            .fold(0u32, |t, &l| t + u32::from(l))
    );
}
