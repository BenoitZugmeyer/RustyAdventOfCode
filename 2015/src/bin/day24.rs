...
fn pwet(
    ...
    for (index, weight) in package_weights.iter().enumerate() {
        ...
        result = if sum + weight == group_weight {
            ...
            min_configuration(
                result,
                pwet(group_weight, &package_weights[index + 1..], group, step + 1),
            )
        };
...
fn find_ideal_configuration(
    package_weights: &mut Vec<u64>,
    groups_count: u64,
) -> Result<(usize, u64), String> {
    let sum: u64 = package_weights.iter().sum();
    if sum % groups_count != 0 {
        return Err(format!(
            "Total weight is not a multiple of {}",
            groups_count
        ));
    }
...
fn example1() {
    ...
    assert_eq!(
        find_ideal_configuration(&mut package_weights, 3),
        Ok((2, 99))
    );
}
...
fn example2() {
    let mut package_weights = (1..6).chain(7..12).collect::<Vec<u64>>();
    assert_eq!(
        find_ideal_configuration(&mut package_weights, 4),
        Ok((2, 44))
    );
}

fn main() {
    let mut package_weights = io::stdin()
        .lock()
        .lines()
        .filter_map(|l| l.ok())
    ...
    match find_ideal_configuration(&mut package_weights, 3) {
        Ok((_, qe)) => println!(
            "3 packages: quantum entanglement of the first group of packages: {}",
            qe
        ),
    ...
    match find_ideal_configuration(&mut package_weights, 4) {
        Ok((_, qe)) => println!(
            "4 packages: quantum entanglement of the first group of packages: {}",
            qe
        ),
        Err(s) => println!("Error: {}", s),
    }
